...
### Frontend
- Criação de componente para listagem de pacientes
- Renderização de tabela utilizando Angular Material (MatTable)
- Mock de dados para exibição inicial dos pacientes
- Criação de service no frontend preparado para consumo de API REST
- Estrutura pronta para integração com endpoints do backend

### Backend
- Estruturação do backend com NestJS
- Configuração do Prisma ORM
- Modelagem inicial para entidade de pacientes
- Criação de endpoints REST (não funcionais no momento)
- Estrutura de services e controllers implementada

### Observações Técnicas
- Durante a integração do backend, ocorreram conflitos relacionados à configuração de módulos ES
- Apesar da estrutura estar criada, não foi possível finalizar a comunicação entre frontend e backend dentro do prazo do desafio

### Próximos Passos
- Ajustar configuração de módulos (ESM/CommonJS) no backend
- Finalizar implementação dos endpoints
- Conectar frontend ao backend para consumo real dos dados
--------
